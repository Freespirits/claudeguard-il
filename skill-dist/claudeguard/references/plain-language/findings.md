...
**EN:** A component in your app is marked "exported" with no permission protecting it — meaning any other app installed on the same device can invoke it directly. A malicious app installed alongside yours can reach this component without the user even knowing, bypassing whatever your UI would have required. If the component is a content provider, it can even read or write data directly. The fix: add a permission (android:permission) to the component, or turn off the export if it is not needed. This is never reported for the app's launcher screen — that one has to be exported, and turning it off would stop your app from opening at all.

## CG-AND-005
**HE:** קובץ הגדרות הרשת של האפליקציה (network_security_config.xml) מורה לה לסמוך על תעודות אבטחה שבעל המכשיר התקין בעצמו, ולא רק על אלה שמגיעות עם מערכת ההפעלה. המשמעות: כל מי שמצליח להתקין תעודה על המכשיר — תוכנת ניטור, פרופיל ניהול, או דף שמבקש מהמשתמש "להתקין אישור כדי להמשיך" — יכול לקרוא ולשנות את כל התעבורה המוצפנת של האפליקציה, ושום דבר בחיבור לא ייראה חשוד. הפתרון: להשאיר רק ‎<certificates src="system" />‎, ואם צריך תעודות משתמש לפיתוח — להעביר אותן לתוך ‎<debug-overrides>‎, שלא נכנס לגרסה שמתפרסמת.
**EN:** The app's network configuration file (network_security_config.xml) tells it to trust security certificates that the phone's owner installed, not only the ones that ship with the operating system. That means anyone who can get a certificate onto the device — monitoring software, a management profile, or a page that asks the user to "install a certificate to continue" — can read and change all of the app's encrypted traffic, and nothing about the connection will look wrong. The fix: keep only `<certificates src="system" />`, and if you need user certificates for development, move them inside `<debug-overrides>`, which is not compiled into the published build.
