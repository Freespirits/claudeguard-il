...
הזרוע המואצלת, במדידה ראשונה עד כה (semgrep 1.172.0, `--config auto`, הרישום של 2026-07-30): **3/9
זוהו — כולם הזרקת פקודות — ו-0/9 אושרו**, האחרון מכוון: CG-SAST-001 הוא מקור שיפוט ולא יכול להגיע
ל-`confirmed`. ששת הפספוסים הם מקרי העומק שסט הכללים `auto` של הרישום לא ממדל — SSTI ב-
`render_template_string` של Jinja2, ‎`$where` ב-MongoDB, ‏`autoescape:false` ב-Swig, SSRF דרך
`needle`, SQL בבניית מחרוזת, ושרשור shell אחד ב-`popen`. כשהמואצל רץ, אותו כרטיס ניקוד קורא
**14/25 (56%)** בהיקף ו-**14/28 (50%)** על כל התיוגים — semgrep גם תפס תיוג אחד בקטגוריה משלנו
שהכללים פספסו (cleartext-transit ב-NodeGoat) — וחשדות להתרעת שווא נשארו 0. המספר הזה **נסחף עם
גרסת semgrep ועם הרישום המרוחק**, וזו בדיוק הסיבה ש-`--sast` נשאר אופציונלי ושהמדידה נושאת את
התאריך שלה במקום להיכנס לכותרת. לשחזור: `node bench/wild.mjs --sast` ממכונה שמגיעה ל-semgrep.dev.

הבנצ'מרק נהג להדפיס את התשעה האלה כ"אין כלל לקטגוריה הזו" ולדווח רק על 63% — שניהם בוטלו כ-**ERR-008**
ב-[`ERRATA.md`](ERRATA.md).
